...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        if let image = UIImage(named: "Oddstream-background") {
            UIGraphicsBeginImageContext(CGSize(width: 1.0, height: (window?.bounds.size.height)!))
            image.drawInRect(CGRect(x: 0.0, y: 0.0, width: 1.0, height: (window?.bounds.size.height)!))
